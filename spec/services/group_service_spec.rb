...
describe GroupService do
  ...
  let(:group) { double 'group', :set_fields => nil, :save => nil }
  ...
  describe 'when associating group' do
    ...
    describe 'when result group matching specified result group id is not present' do
      it 'raises ArgumentError' do
        expect{ subject.associate_group(result_group_id: 2, result_groups: [], library_album_id: 3) }.to raise_error(ArgumentError)
      end
    end
