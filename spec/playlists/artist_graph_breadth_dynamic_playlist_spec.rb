...
require 'errors'
require 'playlists/artist_graph_breadth_dynamic_playlist'

describe Dabster::Playlists::ArtistGraphBreadthDynamicPlaylist do
    ...
    context 'when constructed with playlist having initial artist' do
      ...
      context 'when item has been selected' do
        ...
        context 'when last played item has artists available for selection' do
          ...
          before { allow(recommender).to receive(:select_artists_by_artist).with('initial_artist').and_return(['artist0', 'artist1']) }
        ...
        context 'when last played item has no artists available for selection' do
          ...
          before { allow(recommender).to receive(:select_artists_by_artist).with('initial_artist').and_return([]) }
