module Dabster
  class LibraryAlbumsController < ApplicationController
    def show
      @library_album = Dabster::Library::Album.first! id: params[:id]
      ...
      if @group = @library_album.group
        ...
        @group = Dabster::Group.new
      end
    end

    def index
      @query_options = OpenStruct.new(params[:query_options])
      ds = Dabster::Library::Album.dataset
    ...
    def make_whatcd_request
      @whatcd_response = Dabster::Whatcd::TorrentGroup.search(params[:whatcd_request])
