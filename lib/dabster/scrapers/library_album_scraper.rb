module Dabster
  module Scrapers
    class LibraryAlbumScraper
      ...
      def initialize
        @group_service = Dabster::Services::GroupService.new(Dabster::Group)
        @artist_service = Dabster::Services::ArtistService.new(Dabster::Artist)
      end
      ...
      def fuzzy_match(album)
        ...
        # Find the group
        torrent_group = Dabster::Whatcd::TorrentGroup.find(id: matching.id)
      ...
      def id_match(album, torrent_group_id, confidence)
        torrent_group = Dabster::Whatcd::TorrentGroup.find(id: torrent_group_id)
      ...
      def hash_match(album, hash)
        torrent_group = Dabster::Whatcd::TorrentGroup.find(hash: hash)
      ...
      def search(filter)
        Dabster::Whatcd::TorrentGroup.search(filter)
      end
